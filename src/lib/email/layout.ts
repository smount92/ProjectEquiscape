 ...
 * wrap them with Resend I/O. The two pieces of send-time config every
 * one of those senders needs (the From address, the origin) live here
 * too, because "here" is the one module all four of them already
 * import.
 *
 * EVERY card is built twice: renderBrandedEmail for the HTML part and
 * renderBrandedEmailText for the plain-text part. Sending html with no
 * text/plain alternative is read by spam filters as a signal, and it is
 * the reason mail from the Hub lands in Promotions.
 */

const APP_URL = process.env.NEXT_PUBLIC_APP_URL || "https://modelhorsehub.com";

/**
 * The From address for every Resend send on the site. Was copy-pasted,
 * fallback literal and all, into four files — so a domain change was
...
const ENTITIES: [RegExp, string][] = [
    ...
    [/&lt;/g, "<"],
    [/&gt;/g, ">"],
    // Last, always: unescaping it earlier would let "&amp;lt;" become "<".
    [/&amp;/g, "&"],
];

/**
 * Turn one of THIS module's card bodies into readable text.
 *
 * Deliberately not a general-purpose HTML converter — it handles the
 * small, known set of tags the builders above emit, and it converts
 * structure rather than deleting it: list items keep their bullet,
 * paragraphs keep their blank line, the stat table becomes a label/value
 * pair per line. A regex that only stripped angle brackets would produce
 * one run-on paragraph, which is worse than no text part at all because
 * it looks like the email was generated by something broken.
 */
export function htmlToPlainText(html: string): string {
    let text = html
        // The builders above are written as indented template literals, so
        // the whitespace BETWEEN tags is layout, not content. Drop it first
        // or every pretty-printed newline becomes a blank line.
        .replace(/>\s+</g, "><")
        .replace(/<br\s*\/?>/gi, "\n")
        // The bullet opens the item, </li> ends it — one line each.
        .replace(/<li[^>]*>/gi, "· ")
        .replace(/<\/li\s*>/gi, "\n")
        // Hard breaks: a paragraph, a finished list, a heading, a quote.
        .replace(/<\/(p|ul|ol|table|h[1-6]|blockquote)\s*>/gi, "\n\n")
        // Soft breaks: the layout <div>s and table cells.
        .replace(/<\/(div|tr|td)\s*>/gi, "\n")
        .replace(/<[^>]+>/g, "");
    for (const [pattern, char] of ENTITIES) text = text.replace(pattern, char);
    return text
        .split("\n")
        .map((line) => line.replace(/[ \t]+/g, " ").trim())
        // Any run of blank lines becomes exactly one.
...
/**
 * The same card as renderBrandedEmail, as text/plain. Takes the very
 * same input object so the two parts can never drift into telling
 * different stories — build the object once, render it twice.
 */
export function renderBrandedEmailText(input: BrandedEmailInput): string {
    ...
    if (input.ctaLabel && input.ctaUrl) {
        lines.push("", `${input.ctaLabel}: ${absoluteUrl(input.ctaUrl)}`);
    }

    lines.push("", "--", input.footerNote);
    if (!input.hideSettingsLink) {
        lines.push(`Notification settings: ${APP_URL}/settings`);
    }
    lines.push(APP_URL);
  ...
  <div style="max-width:560px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="text-align:center;margin-bottom:28px;">
      <h1 style="margin:0;font-size:22px;font-weight:700;letter-spacing:0.5px;">
        <img src="${APP_URL}/logo.png" alt="MHH" width="32" height="32" style="vertical-align:middle;margin-right:8px;" />
