/**
 * ISO 3166-1 alpha-2 countries — for a member's "where I am" flag on
 * listings, passports and profiles (migration 218). Country only, never
 * a city: the point is "is this seller on my side of an ocean", which is
 * what decides a purchase in a hobby that ships resin in boxes.
 *
 * The flag is the two regional-indicator glyphs; platforms without emoji
 * flags (Windows) show the letters, so the name is always printed too.
 */
...
export const COUNTRIES: readonly Country[] = PAIRS.map(([code, name]) => ({ code, name })).sort((a, b) =>
    a.name.localeCompare(b.name),
);

const NAME_BY_CODE = new Map(PAIRS);

export function isCountryCode(value: unknown): value is string {
    return typeof value === "string" && NAME_BY_CODE.has(value.toUpperCase()) && value.length === 2;
}

export function countryName(code: string | null | undefined): string | null {
    if (!code) return null;
    return NAME_BY_CODE.get(code.toUpperCase()) ?? null;
}

/** The two regional-indicator glyphs, e.g. "PL" → 🇵🇱. Null for an unknown code. */
