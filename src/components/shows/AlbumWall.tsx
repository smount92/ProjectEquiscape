                    ...
                    {tiles.map((tile) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* SHARE-YOUR-PLACING chip — placed tiles only,
                                    post-results. Links the entry's public
                                    celebration page (rosette OG card); sits
                                    below the vote badge when one renders. */}
                                {resultsPublished && entry.place !== null && (
                                    <Link
                                        href={`/shows/${showId}/placing/${entry.id}`}
                                        className={`absolute right-1.5 inline-flex items-center justify-center rounded-full bg-black/65 p-1.5 text-white no-underline transition-colors hover:bg-black/85 ${
                                            votingEnabled ? "top-10" : "top-1.5"
                                        }`}
                                        aria-label={`Share ${entry.horseName}'s ${placeLabel(entry.place)} placing`}
                                        title="Share this placing"
                                        data-testid="wall-share-chip"
                                    >
                                        <svg
                                            width="14"
                                            height="14"
