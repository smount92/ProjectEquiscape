...
export default function LikeToggle({ initialLiked, initialCount, onToggle }: LikeToggleProps) {
 ...
 return (
 <button
 className={`text-muted-foreground inline-flex min-h-[44px] cursor-pointer items-center gap-1 rounded-sm border-none bg-transparent px-1.5 py-0.5 text-sm transition-colors hover:bg-card sm:min-h-0 ${liked ? "text-rose-500" : ""}`}
 onClick={handleClick}
 disabled={isPending}
