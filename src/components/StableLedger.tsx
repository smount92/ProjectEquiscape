...
function getFinishBadgeClass(finishType: string): string {
 ...
 switch (finishType) {
 case"OF":
 return `${base} bg-[rgba(59,130,246,0.15)] text-[rgb(59,130,246)]`;
 case"Custom":
 return `${base} bg-[rgba(168,85,247,0.15)] text-[rgb(168,85,247)]`;
 case"Artist Resin":
 return `${base} bg-[rgba(236,72,153,0.15)] text-[rgb(236,72,153)]`;
 default:
 return `${base} bg-surface-secondary text-ink-light`;
 }
}
...
export default function StableLedger({
 ...
 className="hover:text-forest cursor-pointer transition-colors select-none"
 >
 Name{sortIndicator("name")}
 </th>
 <th
 onClick={() => toggleSort("ref")}
 className="hover:text-forest cursor-pointer transition-colors select-none max-md:hidden"
 >
 ...
 <span className="bg-surface-secondary flex h-10 w-10 items-center justify-center rounded-sm text-xl">
 🐴
 </span>
 )}
 </Link>
 ...
 className="text-ink hover:text-forest font-semibold no-underline transition-colors"
 >
 {horse.customName}
 </Link>
 </td>
 <td className="text-ink-light max-w-[180px] overflow-hidden text-ellipsis max-md:hidden">
 {horse.refName}
 </td>
 <td>
 ...
 <td className="text-ink-light text-xs max-md:hidden">
 {horse.collectionName ? `📁 ${horse.collectionName}` :"—"}
 </td>
 <td className="max-md:hidden">
 {horse.tradeStatus ==="For Sale" && (
 <span className="inline-block rounded-full bg-[rgba(34,197,94,0.15)] px-1.5 py-[2px] text-xs font-medium text-[rgb(34,197,94)]">
 💲 For Sale
 </span>
 )}
 {horse.tradeStatus ==="Open to Offers" && (
 <span className="inline-block rounded-full bg-[rgba(251,191,36,0.15)] px-1.5 py-[2px] text-xs font-medium text-[rgb(202,138,4)]">
 🤝 Offers
 </span>
 )}
