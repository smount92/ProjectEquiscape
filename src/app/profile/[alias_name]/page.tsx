...
import {
    ...
    fetchProfileSeason,
    fetchStableTitles,
} from "@/app/profile/reads";

/**
 * A member's profile — the page every Members-directory card points at.
 *
 * WHAT IT IS ALLOWED TO SAY ABOUT SOMEONE ELSE
 * --------------------------------------------
 * Every read on this page is scoped to what is legally public about
 * the subject, and the audit behind that lives in the header of
 * src/app/profile/reads.ts. Two rules are load-bearing and must not
 * be relaxed without redoing that audit:
 *   * the barns list filters on `groups.visibility = 'public'` in
 *     app code, never on RLS or the `is_private` boolean;
 *   * the posts list runs the feed's `isGloballyVisible` rule,
 *     because `posts_select` admits a private barn's posts to that
 *     barn's members and this is a public page.
 * 🔒 financial_vault is never queried here.
 *
 * MODERATION STATE. Deleted and suspended accounts are resolved
 * through the service-role client, because `is_suspended` was added
 * to `users` after migration 133's column-GRANT sweep and is not
 * selectable — or filterable — by an RLS-scoped client at all.
 */
...
export default async function ProfilePage({
    ...
    // Tombstoned accounts are gone for everyone. The anon path already
    // enforced this; the member path did not, so a deleted member's
    // stable stayed browsable to anyone signed in.
    if (profileUser.account_status === "deleted") {
        notFound();
    }
    ...
    // Suspension is invisible to an RLS-scoped client (see the file
    // header), so it is read with the service role — the same reason
    // AnonProfile reads account_status that way.
    ...
    if (!isOwnProfile) {
        ...
        isSuspended = modRow?.is_suspended === true;
    }
    ...
    // A suspended member's profile becomes a plaque and nothing else.
    // Their stable, record and words come down for the duration; the
    // page still resolves so inbound links do not 404 into confusion.
    if (isSuspended) {
        return (
            <div className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6 md:py-10 lg:px-8">
                <ProfileMasthead
                ...
                />
                <div className="mt-6">
                    <EmptyNote icon="⛔" title="This account is suspended">
                        Their stable and show record are hidden while a moderator reviews the
                        account. Nothing here reflects on the horses themselves.
                    </EmptyNote>
                </div>
            </div>
        );
    ...
    const rootStyle = themeStyle(custom.theme);

    let bannerUrl: string | null = null;
    if (custom.bannerPath) {
        const { data: signedBanner } = await supabase.storage
            .from("avatars")
            .createSignedUrl(custom.bannerPath, 3600);
        bannerUrl = signedBanner?.signedUrl ?? null;
    }
    ...
    // Studio link — gated on portfolio_visible so the profile and the
    // artist directory (browseArtists filters the same way) agree about
    // who is listed.
    const { data: studioProfile } = await supabase
        .from("artist_profiles")
        .select("studio_slug, studio_name, portfolio_visible")
        .eq("user_id", profileUser.id)
        .maybeSingle();
    const studioVisible = studioProfile?.portfolio_visible !== false;
    const studioSlug = studioVisible ? studioProfile?.studio_slug || null : null;
    const studioName = studioProfile?.studio_name || null;

    // Block status. `isBlocked` reads the viewer's OWN blocks only —
    // "who blocked me" is structurally unreadable — so this means
    // "I blocked them", and it hides their words from me, matching the
    // one-directional rule the feed and the members directory use.
    ...
    const showRecords = (rawRecords ?? []).map((r) => ({
        ...
        horseId: r.horse_id,
        horseName:
            (r.user_horses as unknown as { custom_name: string } | null)?.custom_name ?? "—",
    ...
    // ================================================================
    // STARS OF THE STABLE — the member's own pick when they have made
    // one (customization `featured`), otherwise auto-picked: the 3
    // public horses with the most show records, falling back to the
    // most recently added.
    // ================================================================
    ...
    const autoPicked = custom.featured.length === 0;
    const starIds: string[] = autoPicked
        ? [...recordCountByHorse.entries()]
              .sort((a, b) => b[1] - a[1])
              .slice(0, 3)
              .map(([id]) => id)
        : [...custom.featured];
    ...
    const allCards = horses.map(buildCard);
    const extraStarCards = extraStarHorses.map(buildCard);
    // A hand-picked shelf leads the stable too, not just the star row.
    const profileCards = applyFeaturedOrder(allCards, custom.featured);
    ...
    const forSaleCards = profileCards.filter(
    ...
    // because a block is "I don't want to read them", not "they were
    // never at that show".
    const [season, stableTitles, barns, posts] = await Promise.all([
        sectionVisible(custom, "season")
            ? fetchProfileSeason(supabase, profileUser.id)
            : Promise.resolve(null),
        sectionVisible(custom, "season")
            ? fetchStableTitles(
                  supabase,
                  allCards.map((c) => ({ id: c.id, customName: c.customName })),
              )
            : Promise.resolve([]),
        sectionVisible(custom, "barns")
            ? fetchProfileBarns(supabase, profileUser.id)
            : Promise.resolve([]),
        sectionVisible(custom, "posts") && !blocked
            ? fetchProfilePosts(supabase, profileUser.id)
            : Promise.resolve([]),
    ]);
        ...
        >
            ...
            {horse.tradeStatus === "For Sale" && (
                <div className="mt-1 text-center">
                    <span className="stamp stamp-red">
                        For Sale
                        {horse.listingPrice ? ` $${horse.listingPrice.toLocaleString("en-US")}` : ""}
                    </span>
                </div>
            )}
            ...
            {(horse.tradeStatus === "For Sale" || horse.tradeStatus === "Open to Offers") &&
                horse.marketplaceNotes && (
                    <div
                        className="mt-1 truncate px-1 text-center text-[0.65rem] text-muted-foreground"
    ...
    return (
        ...
        >
            ...
            <ProfileMasthead
                ...
                actions={
                    <>
                        ...
                        {isOwnProfile && (
                            <Link href="/profile/customize" className="btn-ghostleather">
                                🎨 Make it yours
                            </Link>
                        )}
            ...
            {/* ── GREEN STRAP STATS ── */}
            <div
                className="stats-strap relative -mt-4 mx-6 sm:mx-10"
                role="group"
                aria-label="Stable statistics"
            >
                {strapStats.map((stat) => (
                    <div key={stat.label}>
            ...
            {season && sectionVisible(custom, "season") && (
                <ChampionshipLine
                    alias={profileUser.alias_name}
                    isOwnProfile={isOwnProfile}
                    season={season}
            ...
            {sectionVisible(custom, "stars") && starCards.length > 0 && (
                <section className="animate-fade-in-up mt-10" id="stars">
                    <SectionHeading
                        title="Stars of the Stable"
                        note={autoPicked ? "auto-picked from show records" : "hand-picked"}
                    />
                    <div className="shelfwrap">
                        {/* .shelfrow is a fixed 3-up grid — right for the
                            auto-pick, wrong for a hand-picked six. Past
                            three, the scrolling shelf (same wood, same
                            edge) carries them instead of stacking a
                            second row onto one shelf board. */}
                        <div
                            className={starCards.length > 3 ? "shelf-strip" : "shelfrow"}
                            {...(starCards.length > 3
                                ? {
                                      tabIndex: 0,
                                      role: "region",
                                      "aria-label": "Featured horses shelf",
                                  }
                                : {})}
                        >
                            {starCards.map((star) => (
                                <Link
                                    ...
                                    className={
                                        starCards.length > 3 ? "polaroid w-[220px]" : "polaroid"
                                    }
                                ...
                                >
                                    ...
                                    {star.starLine && (
                                        <div className="polaroid-star">{star.starLine}</div>
                                    )}
                                </Link>
                            ))}
                        </div>
            ...
            {/* Trophy Case — the member's show_badges pref gates it, and
                the section toggle can retire it entirely. Owner always
                sees their own. */}
            {userBadges.length > 0 &&
                sectionVisible(custom, "trophies") &&
                (isOwnProfile || (profileUser.show_badges ?? true)) && (
                    <section className="animate-fade-in-up mt-10" id="trophies">
                        <SectionHeading title="🏆 Trophy Case" />
                ...
                {profileCards.length === 0 ? (
                    <EmptyNote
                        ...
                        action={
                            isOwnProfile ? (
                                <Button asChild>
                                    <Link href="/dashboard">🏠 Go to My Stable</Link>
                                </Button>
                            ) : undefined
                        }
                    >
                    ...
                    <div className="shelfwrap">
                        <div
                            ...
                            tabIndex={0}
                            role="region"
                            aria-label="Public horses shelf"
                        >
            ...
            {/* ── ON THE BLOCK — what they're selling ── */}
            {sectionVisible(custom, "market") && (
                <ForSaleStrip
                    alias={profileUser.alias_name}
                    isOwnProfile={isOwnProfile}
                    cards={forSaleCards}
            ...
            {sectionVisible(custom, "ledger") && recentRecords.length > 0 && (
                <section className="animate-fade-in-up mt-10" id="show-ledger">
                    <SectionHeading title="Recent Show Record" />
                                    ...
                                    {recentRecords.map((rec) => (
                                        <tr key={rec.id}>
                                            ...
                                            <td className="px-2.5 py-2 text-secondary-foreground">
                                                {rec.showId ? (
                                                    <Link
                                                        href={`/shows/${rec.showId}`}
                                                        className="text-secondary-foreground no-underline hover:text-forest hover:underline"
                                                    >
                                                        {rec.showName}
                                                    </Link>
                                                ) : (
            ...
            {/* Reviews Section */}
            {sectionVisible(custom, "reviews") && ratingSummary.count > 0 && (
